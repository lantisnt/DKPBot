...
class DKPBot:
    ...
    def _configure(self):
        ...
        self.__announcement_mention_role = int(
            self.__config.guild_info.announcement_mention_role
        )
        ...
        self.__direct_message_response = bool(
            self.__config.guild_info.direct_message_response
        )
        self.__block_response_modifier = bool(
            self.__config.guild_info.block_response_modifier
        )
    ...
    def get_announcement(self):
        ...
        if self.is_announcement_mention_role_set():
            return (
                "<@&{0}>".format(self.__announcement_mention_role),
                BasicAnnouncement(announcement).get(),
            )
        else:
    ...
    def _set_channel_team_mapping(self, channel_id, team):
        ...
        # Limit to 8
        in_limit = True
        if (len(self._channel_team_map) == 8) and (
            str(channel_id) not in self._channel_team_map
        ):
            (channel, team) = self._channel_team_map.popitem(False)
            BotLogger().get().info(
                "Removing mapping of team %s from channel %s", team, channel
            )
            in_limit = False
    ...
    def _parse_player_param(self, param):
        ...
        BotLogger().get().debug(
            "Parse player param results. [targets: %s] [aliases: %s] [original: %s] [int_list: %s]",
            targets,
            aliases,
            original,
            int_list,
    ...
    def __handle_command(self, command, param, request_info):
        ...
        if param is not None:
            ...
            if sanitized_command in ["display", "config"]:
                param = ""
            else:
        ...
        if callback and callable(callback):
            ...
            BotLogger().get().info(
                "Calling [%s] with param [%s] for [%d]",
                sanitized_command,
                param,
                self.__guild_id,
            )
            bot_memory_manager.Manager().Handle(
                self.__guild_id
            ...
            if sanitized_command not in [
                "config",
                "display",
            ]:  # inject reminder except on config/display calls
            ...
            BotLogger().get().debug(
                "Unknown command [%s] for [%d]", sanitized_command, self.__guild_id
            )
            return Response(ResponseStatus.IGNORE)
        ...
        if len(message) > 0 and message[0] == self.__prefix:
            ...
            if command is not None:
                return self.__handle_command(
                    command.lower(), param, request_info
                )
        ...
        if team is None:
            for team_data in self.__db["group"].values():
                if team_data.get(group):
                    team_data[group].sort(key=lambda info: info.dkp(), reverse=True)
                else:
                        ...
                        if history_entry.dkp() > 0:
                            ...
                            if abs(now - history_entry.timestamp()) <= inactive_time:
                                ...
                                if (
                                    ...
                                    >= self.__POSITIVE_ENTRY_THRESHOLD
                                ):
                                    dkp.set_active()
                                    break
                            else:
    ...
    def build_database(self, input_string, info):
        ...
        if not isinstance(saved_variable, dict):
            ...
            return Response(
                ResponseStatus.SUCCESS,
                BasicCritical(
                    "No SavedVariables found in .lua file. Check if you have provided proper savedvariable file."
                ).get(),
            )
        ...
        if not self._build_config_database(saved_variable):
            BotLogger().get().error(
                "Configuration Database building failed. Please validate your settings."
            )
            return Response(
                ResponseStatus.SUCCESS,
                BasicError(
        ...
        if not self._build_dkp_database(saved_variable):
            ...
            return Response(
                ResponseStatus.SUCCESS,
                BasicError(
                    "Profile Database building failed. Please validate your settings."
                ).get(),
            )
        if not self._build_loot_database(saved_variable):
            ...
            return Response(
                ResponseStatus.SUCCESS,
                BasicError(
                    "Loot Database building failed. Please validate your settings."
                ).get(),
            )
        if not self._build_history_database(saved_variable):
            ...
            return Response(
                ResponseStatus.SUCCESS,
                BasicError(
                    "History Database building failed. Please validate your settings."
                ).get(),
            )
        ...
        BotLogger().get().info(
            "Building complete in {:04.2f} seconds".format(timestamp_now() - start)
        )
        ...
        for team in self.__db["global"]:
            for table in team:
                if len(table) <= 0:
                    ...
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicError("Global Database building failed.").get(),
                    )
        ...
        for team in self.__db["group"]:
            if len(team) <= 0:
                ...
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicError("Group Database building failed.").get(),
                )

        self.__db_loaded = True

        return Response(
            ResponseStatus.SUCCESS, BasicSuccess("Database building complete.").get()
        )
        ...
        if internal_group:
            if hasattr(internal_group, config):
                ...
                if isinstance(new_value, bool):
                    return (new_value and (value in ["true", True])) or (
                        not new_value and (value in ["false", False])
                    )
    ...
    def __set_config_boolean(
        self, group, config, value
    ):  # pylint: disable=unused-argument
        ...
        BotLogger().get().debug(
            "Setting boolean %s %s to %s", group, config, value_sanitized
        )
    ...
    def __generic_response(success, config, value):
        if success:
            return Response(
                ResponseStatus.SUCCESS,
                BasicSuccess(
                    "Successfuly set **{0}** to **{1}**".format(config, value)
                ).get(),
            )
        else:
            return Response(
                ResponseStatus.SUCCESS,
                BasicError(
                    "Unsupported value **{1}** provided for **{0}**".format(
                        config, value
                    )
                ).get(),
            )
    ...
    def _build_help_internal(self, is_privileged, standings, exclude=[]):
        ...
        embed.build(
            ...
            "Supported command groups:".format(self.__prefix, standings.lower()),
            None,
            get_bot_color(),
            None,
        )
    ...
    def _help_internal(self, is_privileged):
        return Response(
            ResponseStatus.SUCCESS, self._build_help_internal(is_privileged, "dkp")
        )
    ...
    def call_config(self, param, request_info):
        ...
        if callback and callable(callback):
            ...
            string += preformatted_block(
                "Usage:     {0}config timezone CanonicalTimezone".format(self.__prefix)
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config guild-name GuildName\nExample:   {0}config guild-name Some Guild".format(
                    self.__prefix
                )
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config smart-roles value".format(self.__prefix)
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config register #channel".format(self.__prefix)
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config announcement #channel @role".format(self.__prefix)
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config prefix *".format(self.__prefix)
            )
            string += preformatted_block("Current:   {0}\n".format(self.__prefix))
            string += preformatted_block(
            ...
            string += preformatted_block(
                "Usage:     {0}config dm-response value".format(self.__prefix)
            )
            string += preformatted_block(
            ...
            string += preformatted_block(
                "Usage:     {0}config block-response-modifier value".format(
                    self.__prefix
                )
            )
            string += preformatted_block(
            ...
            string += preformatted_block(
                "Usage:     {0}config reload".format(self.__prefix)
            )
            ...
            string += preformatted_block(
                "Usage:     {0}config default".format(self.__prefix)
            )
    ...
    def call_display(self, param, request_info):
        if not request_info["is_privileged"]:
            BotLogger().get().warning(
                "Unprivileged access to display: %s", request_info
            )
            return Response(ResponseStatus.IGNORE)
        ...
        if num_params <= 1:
            ...
            string = "```swift\nUsage:\n\n{0}display Category Config Value```".format(
                self.__prefix
            )
            string += "```swift\nExample:\n\n{0}display loot-history multiple-columns True```".format(
                self.__prefix
            )
            ...
            if category in self.__config.get_directly_accessible_configs():
                if self.__set_config(category, config, value):
                    ...
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicSuccess(
                            "Successfuly set **{0} {1}** to **{2}**".format(
                    ...
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicError(
                            "Unsupported value **{2}** provided for **{0} {1}**".format(
                                param[0], param[1], param[2]
                            )
                        ).get(),
                    )
                ...
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicError("Invalid category **{0}**".format(param[0])).get(),
                )
    ...
    def help_call_general(self, is_privileged):  # pylint: disable=unused-argument
        help_string = "Display this help. You can also get it by @mentioning the bot.\n{0}\n".format(
            preformatted_block(self.get_prefix() + "help", "")
        )
        help_string += "Get basic information about the bot.\n{0}\n".format(
            preformatted_block(self.get_prefix() + "info", "")
        )

        return Response(
            ResponseStatus.SUCCESS, self._help_handler_internal("General", help_string)
        )
    ...
    def help_call_dkp(self, is_privileged):  # pylint: disable=unused-argument
        ...
        help_string += "Display current DKP for as many players, classes or aliases mixed together as you wish.\n{0}".format(
            preformatted_block(
                "{0}dkp class/alias/player\nExamples:\n{0}dkp hunter tanks joe\n{0}dkp rogue druid\n{0}dkp joe andy".format(
                    self.get_prefix()
                ),
                "",
            )
        ...
        return Response(
            ResponseStatus.SUCCESS, self._help_handler_internal("DKP", help_string)
        )
    ...
    def help_call_items(self, is_privileged):  # pylint: disable=unused-argument
        ...
        help_string += "Get value (min, max, average, total) data for loot matching `name`. Supports partial match.\n{0}\n".format(
            preformatted_block(self.get_prefix() + "value name", "")
            + preformatted_block("Supporter only command", "css")
        )
    ...
    def help_call_administration(
        ...
        if is_privileged:
            ...
            return Response(
                ResponseStatus.SUCCESS,
                self._help_handler_internal("Administration", help_string),
            )
        else:
            BotLogger().get().warning(
                "Unprivileged access to help administration: %s", request_info
            )
            return Response(ResponseStatus.IGNORE)

    ### Config handlers ###

    def config_call_bot_type(
        ...
        if num_params == 2:
            ...
            if value in ["clm", "community", "monolith", "essential", "cepgp", "rclc"]:
                ...
                if value == current:
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicSuccess("Retaining current bot type").get(),
                    )
                ...
                if new == value:
                    ...
                    BotLogger().get().error(
                        "Unexpected error during bot type setup: %s", request_info
                    )
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicCritical("Unexpected error during bot type setup").get(),
                    )
            else:
                BotLogger().get().warning("Unsupported bot type %s", value)
                return Response(
                    ResponseStatus.SUCCESS, BasicError("Unsupported bot type").get()
                )
        else:
            return Response(
                ResponseStatus.SUCCESS,
                BasicSuccess("Invalid number of parameters").get(),
            )

    def config_call_prefix(
        self, params, num_params, request_info
    ):  # pylint: disable=unused-argument
        if num_params == 2:
            value = params[1]
            if value in type(self).get_supported_prefixes():
                ...
                if new == value:
                    ...
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicSuccess("Set prefix to `{0}`".format(value)).get(),
                    )
                else:
                    BotLogger().get().error(
                        "Unexpected error during prefix change: %s", request_info
                    )
                    return Response(
                        ResponseStatus.SUCCESS,
                        BasicCritical("Unexpected error during prefix change").get(),
                    )
            else:
                BotLogger().get().warning("Unsupported prefix %s", value)
                return Response(
                    ResponseStatus.SUCCESS, BasicError("Unsupported prefix").get()
                )
        else:
            return Response(
                ResponseStatus.SUCCESS, BasicError("Invalid number of parameters").get()
    ...
    def config_call_register(
        ...
        return Response(
            ResponseStatus.SUCCESS,
            BasicSuccess(
                "Registered to expect Saved Variable lua file on channel <#{0}>".format(
                    channel
                )
            ).get(),
        )

    def config_call_announcement(
        self, params, num_params, request_info
    ):  # pylint: disable=unused-argument
        channel = request_info["channel"]["id"]
        if len(request_info["mentions"]["channels"]) > 0:
        ...
        if len(request_info["mentions"]["roles"]) > 0:
            ...
            role_response = "<@&{0}> will be mentioned in the announcement.".format(
                role
            )

        self.__register_announcement(channel, role)
        response = (
            "Registered channel <#{0}> to announce updated DKP standings.\n".format(
                channel
            )
        )
        ...
        if num_params >= 3:
            ...
            if side not in ["alliance", "horde"]:
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicError(
                        "Last parameter must be either `Alliance` or `Horde`"
                    ).get(),
                )
            ...
            if len(value) > 50:
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicError("Data should not exceed 50 characters.").get(),
                )
            ...
            if new == value:
                ...
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicSuccess(
                        "Server-side data set to `{0} {1}`".format(server, side)
                    ).get(),
                )
            else:
                BotLogger().get().error(
                    "Unexpected error during server-side change: %s", request_info
                )
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicCritical("Unexpected error during server-side change").get(),
                )
        else:
            return Response(
                ResponseStatus.SUCCESS, BasicError("Invalid number of parameters").get()
    ...
    def config_call_guild_name(
        ...
        if num_params >= 2:
            ...
            if new == value:
                ...
                return Response(
                    ResponseStatus.SUCCESS,
                    BasicSuccess("Guild Name set to `{0}`".format(value)).get(),
                )
            else:
                BotLogger().get().error(
    ...
    def config_call_team(
        ...
        if num_params == 1:
            ...
            if self._set_channel_team_mapping(channel, teamId):
                ...
                error_text = (
                    "Exceeded maximum number of channels. Removing oldest assignment."
                )
            return Response(
                ResponseStatus.SUCCESS,
                BasicSuccess(
    ...
    def config_call_dm_response(
        self, params, num_params, request_info
    ):  # pylint: disable=unused-argument
        if num_params == 2:
            success = self.__set_config_boolean(
                "guild_info", "direct_message_response", params[1]
            )
    ...
    def config_call_block_response_modifier(
        self, params, num_params, request_info
    ):  # pylint: disable=unused-argument
        if num_params == 2:
            success = self.__set_config_boolean(
                "guild_info", "block_response_modifier", params[1]
            )
            return type(self).__generic_response(
                success, "block-response-modifier", params[1]
            )
        else:
            return Response(
                ResponseStatus.SUCCESS, BasicError("Invalid number of parameters").get()
