...
async def discord_attachment_check(
    bot: dkp_bot.DKPBot, message: discord.Message, author: str, announce: bool
):
    if len(message.attachments) > 0:
        for attachment in message.attachments:
            if (
                ...
                info = {
                    "comment": discord.utils.escape_markdown(message.clean_content)[
                        :75
                    ],
            ...
            if response.direct_message:
                if isinstance(response_channel, discord.DMChannel):
                    ...
                    BotLogger().get().warning(
                        "Unable to respond on DM to message %s", message
                    )
...
async def on_ready():
    try:
        ...
        BotLogger().get().info(
            "Starting initializing bot for {0} servers".format(len(client.guilds))
        )
